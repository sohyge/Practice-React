...
const LOCATION_INFO_URL = `http://data.ex.co.kr/openapi/safeDriving/forecast?key=${KEY}&type=json`;
...
export const getLocationInfo = async () => {
  const { data } = await axios.get<TrafficInfoResponse>(LOCATION_INFO_URL);
  console.log('suc', data);
  return data;
};
